...
class SpeakScreenState extends ConsumerState<SpeakScreen>
  ...
  Widget build(BuildContext context) {
    final egoListAsync = ref.watch(
      egoModelsFromChatRoomsWithoutRefProvider(chatRoomList),
    );

    return egoListAsync.when(
      data: (egoList) {

        if (egoList.isNotEmpty && selectedEgo == null) {
      ...
      error: (error, stack) {
        return Center(
          child: ElevatedButton(
            onPressed: () {
              ref.refresh(egoModelsFromChatRoomsWithoutRefProvider(chatRoomList));
            },
            child: Text('다시 시도'),
          ),
        );
