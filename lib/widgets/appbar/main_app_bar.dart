...
class MainAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: AppBar(
        title: DefaultTabController( // TabBar를 AppBar의 좌측에 배치하기 위함
          length: tabController.length, // 이용할 Tab은 2개로 지정
          child: TabBar(
            controller: tabController,
            isScrollable: true, // TabBar의 너비를 지정할 수 있게 한다.(Default: Maximum Width)
            tabAlignment: TabAlignment.start, // TabBar의 Tab들을 왼쪽으로 정렬시킨다. (Default: Center)
            labelPadding: EdgeInsets.symmetric(horizontal: 8.0), // 각 텍스트 간의 간격 지정
            indicator: BoxDecoration(), // 선택된 페이지의 밑줄 하이라이팅 제거
            labelStyle: TextStyle( // 선택된 Tab의 텍스트 속성
              color: AppColors.black,
              fontSize: 21,
              fontWeight: FontWeight.bold,
            ),
            unselectedLabelStyle: TextStyle( // 선택되지 않은 Tab의 텍스트 속성
              color: AppColors.gray400,
              fontSize: 21,
              fontWeight: FontWeight.bold,
            ),
            tabs: [ // 지정된 Tab * 알림과 설정은 탭이 아닌 버튼을 통해 다른 페이지로 이동한다.
              Tab(text: "스피크"),
              Tab(text: "캘린더"),
            ],
          ),
        ),
        actions: [ // '알림'과 '설정'을 AppBar의 우측에 배치하기 위한
          Row(
            children: [
              IconButton( // 알림 페이지로 이동하기 위한 Button
                icon: Icon(Icons.alarm), // TODO: 디자인 확정되면 변경할 것
                iconSize: 30,
                onPressed: alertMethod,
              ),
              IconButton( // 설정 페이지로 이동하기 위한 Button
                icon: ImageIcon(Image.asset( // TODO: 이미지 리소스 조회 방식에 맞춰 변경할 것
                  egoIconPath,
                  scale: 9.0,
                ).image),
                iconSize: 30,
